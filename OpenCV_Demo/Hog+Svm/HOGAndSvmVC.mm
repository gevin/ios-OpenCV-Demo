...
- (void)hogSvmTraining
{
    ...
    printf("get negative image name list.\n");
    // 讀取負樣本圖片名字清單
    ...
    // 讀取正樣本，生成 hog 描述子
    for ( int index=0 ; index<pos_img_names.count ; index++) {
        NSString *img_name = pos_img_names[index];
        printf("+ %s\n",[img_name UTF8String]);
    ...
    printf("extract negative image feature.\n");
    srand(time(NULL));
    // 讀取負樣本，生成 hog 描述子
    for ( int index=0 ; index<neg_img_names.count ; index+=2) {
        NSString *img_name = neg_img_names[index];
        printf("- %s\n",[img_name UTF8String]);
        //-----------------------------------------
        //  load training negative image
        //-----------------------------------------
        UIImage *image = [UIImage imageNamed:img_name];
        if(image == nil){
            printf("image nil\n");
        }
        cv::Mat imgM,imgM_small;
        UIImageToMat(image, imgM);
        cv::cvtColor(imgM, imgM, CV_BGR2GRAY);
        ...
        if( imgM.rows * scale_h > 128.0){
            cv::resize(imgM, imgM_small, cv::Size( 64, imgM.rows * scale_h) );
        }
        else if( imgM.cols * scale_v > 64.0){
            cv::resize(imgM, imgM_small, cv::Size( imgM.cols * scale_v, 128) );
        }
        int x = (imgM_small.cols-64)/2;
        int y = (imgM_small.rows-128)/2;
        imgM_small = imgM_small(cv::Rect( x, y, 64,128) );
        ...
        hog->compute(imgM_small,descriptors,cv::Size(8,8));
        ...
        // save feature descriptor data to training_data
        for(int i=0; i<descriptorDim; i++){
            train_data.at<float>(pos_img_count + index,i) = descriptors[i];
        }
        
        // save -1 label to training_labels
        train_labels.at<float>(pos_img_count + index,0) = -1;//負樣本為-1，無人
    ...
    // SVM參數 SVM類型:C_SVC，ker nel函數:線性，松弛因子C=0.01
    CvSVMParams param(CvSVM::C_SVC,  // SVM 類型，C_SVC，NU_SVC，ONE_CLASS，EPS_SVR，NU_SVR
                      ...
                      0.01,          // C value 懲罰錯誤分類值，SVM類型（C_SVC/ EPS_SVR/ NU_SVR）的参數 C
...
- (void)initPersonDetector
{
    ...
    /*
     ...
     Gevin note:
     目前 train 出來的結果，根本沒辦法用，偵測不到行人，不確定是樣本不夠，還是我參數有問題
     */
...
//    printf("支持向量數：%d\n",supportVectorNum);
//    cv::Mat alphaMat = cv::Mat::zeros(1, supportVectorNum, CV_32FC1);//alpha向量就是權重向量，長度等於支持向量個數
...
- (void)detectPerson:(UIImage*)image
{
    ...
    printf("進行多尺寸檢測\n");
    _hog.detectMultiScale(testImgM_gray, found, 0, cv::Size(8,8), cv::Size(32,32), 1.05, 2);
    printf("找到的矩形框數:%lu\n",found.size());
    // 畫框
